...
for datafile = config.dataFiles
    for ang = config.initialParameters.angles
        for wav = config.initialParameters.wavelength

            data = dlmread(datafile);
            coords = euler(data(:, 1:3), 0, ang, 0);

            basename = split(datafile, ".");
            basename = basename(1, 1);
            mat_fname = './output/' + string(basename) + '_' +...
                        string(ang) + 'deg_' + pol +'pol_' + ...
                        string(wav) + 'wav_' + string(config.initialParameters.beamWidth) + ...
                        'bw.mat';
            ...
            initialField = celes_initialField('polarAngle',     0, ...
                                              'azimuthalAngle', 0, ...
                                              'polarization',   config.initialParameters.polarization, ...
                                              'beamWidth',      config.initialParameters.beamWidth, ...
                                              'focalPoint',     [0, 0, 0] ...
                                              );
            ...
            %
            numerics = celes_numerics('lmax',                       config.numerics.lmax, ...
                                      'polarAnglesArray',           0:pi/5e3:pi, ...
                                      'azimuthalAnglesArray',       0:pi/1e2:2*pi, ...
                                      'gpuFlag',                    config.numerics.gpu, ...
                                      'particleDistanceResolution', 1, ...
                                      'solver',                     solver);
            
            scale = config.solGrid.scale;
            bnd = config.solGrid.size / scale;
            stp = config.solGrid.step / scale;
