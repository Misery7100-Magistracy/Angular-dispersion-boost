...
\begin{figure}[H]
    \subimgtwo[../python/TE_10deg_check]{$\theta = 10^{\circ}$. Efficient scattering at angles $
    \theta_{\textrm{s}} = 0^{\circ}$, $-24^{\circ}$, $135^{\circ}$ with respect to the incidence direction.}{TE_check:a}{0.48\textwidth}
    \hfil
    \subimgtwo[../python/TE_15deg_check]{$\theta = 15^{\circ}$. Efficient scattering at angles $
    \theta_{\textrm{s}} = 0^{\circ}$, $-27^{\circ}$ with respect to the incidence direction.}{TE_check:b}{0.48\textwidth}
...
%Using the example of a cubic lattice location of high-density plasma spheres, options were calculated diffraction control of the tenth laser harmonic for various of the lattice constants and revealed the features of scattering with respect to the angle incidence of radiation on the grating (\autoref{14deg_TM_check:image}). The results show the ability to control high harmonics of laser radiation (XUV range) using an ionized cluster gas.
