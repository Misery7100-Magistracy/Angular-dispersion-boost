...
\begin{figure}[H]
    \subimgtwo[components/img/celes/Es_20nm_15deg_1harm.pdf]{$\lambda = \lambda_{L} = 830$ nm.}{random_ka0.7:a}{0.4\textwidth}
    \subimgtwo[components/img/celes/Es_20nm_15deg_10harm.pdf]{ $\lambda = \lambda_{10} = 83$ nm.}{random_ka0.7:b}{0.4\textwidth}
...
\begin{figure}[H]
    \subimgtwo[components/img/celes/dphi_dtheta_kprime_d_2l_phi0_0_theta0_15.pdf]{Решение \autoref{bragg_wolf_order_spherical} в целых индексах Миллера.}{1st_check_diffrth:a}{0.35\textwidth}
    \hfil
    \subimgtwo[components/img/celes/e_int_20nm_15deg]{ $E_{\textrm{int}}$ по \autoref{e_int}.}{1st_check_diffrth:b}{0.45\textwidth}
    %\\
...
Рассмотрим влияние нерегулярности решетки на наиболее интенсивные направления рассеянного поля, то есть зависимость \autoref{e_int} от нерегулярности решётки $\eta$. Для этого смоделировано рассеяние в случае квазирегулярной решётки при различных параметрах и нерегулярностью $\eta$ от 0 до 0.5 (\Autoref{nonreg_mono:image, nonreg_ka0.7:image}).

    \begin{figure}[H]
        \subimgtwo[components/img/celes/e_int_rad_20nm_20deg_nonreg0.0]{$\eta = 0.0$.}{nonreg_mono:a}{0.45\textwidth}
        \hfil
        \subimgtwo[components/img/celes/e_int_rad_20nm_20deg_nonreg0.1]{$\eta = 0.1$.}{nonreg_mono:b}{0.45\textwidth}
        \subimgtwo[components/img/celes/e_int_rad_20nm_20deg_nonreg0.3]{$\eta = 0.3$.}{nonreg_mono:c}{0.45\textwidth}
        \hfil
        \subimgtwo[components/img/celes/e_int_rad_20nm_20deg_nonreg0.45]{$\eta = 0.45$.}{nonreg_mono:d}{0.45\textwidth}
    ...
    \begin{figure}[H]
        \subimgtwo[components/img/celes/Es_20nm_15deg_10harm_0.1nonreg.pdf]{$|\vectbf{E}{s}|^2$  в плоскости поляризации, сечение $\Delta \varphi = 0$. Границы газового слоя обозначены пурпурным цветом. Амплитуда рассеянного поля нормирована свой максимум.}{nonreg_ka0.7:a}{0.4\textwidth}
        \subimgtwo[components/img/celes/e_int_20nm_15deg_0.1nonreg.pdf]{Полярная диаграмма рассеяния (\autoref{e_int}).}{nonreg_ka0.7:b}{0.4\textwidth}
        \caption{Рассеяние гауссового пучка на слое квазирегулярно расположенных кластеров радиуса $a = 20$ nm, $\theta_0 = 15^{\circ}$, $\varphi_0 = 0^{\circ}$, $\eta = 0.1$.}\label{nonreg_ka0.7:image}
    \end{figure}
...
... про кластеры (\autoref{radius_wawa:image})
    ...
    \begin{figure}[H]
        \subimgtwo[components/img/celes/e_int_20nm_15deg.pdf]{$a = 20$ nm.}{radius_wawa:a}{0.45\textwidth}
        \hfil
        \subimgtwo[components/img/celes/e_int_30nm_15deg.pdf]{$a = 30$ nm.}{radius_wawa:b}{0.45\textwidth}
        \\
        \subimgtwo[components/img/celes/e_int_60nm_15deg.pdf]{$a = 60$ nm.}{radius_wawa:c}{0.45\textwidth}
    ...
    \img[components/img/celes/energy_vs_radius]{Ослабление рассеяния в зависимости от радиуса кластеров, значения нормированы на $E_{\textrm{int}}$ при $a = 20$ nm.}{energy_vs_nonreg:image}{0.5\textwidth}
